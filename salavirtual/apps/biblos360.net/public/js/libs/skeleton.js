...
$(document).ready(function() {
    ...
    $('.cep').mask('00000-000');
    format_cep = function(e) {
        var field = $(this);
        var prefix = field.attr('name').replace(/cep/, '');
        ...
        if (cep && cep.length == 8) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.erro) {
                        $('#' + prefix + 'endereco_sn').val('');
                        $('#' + prefix + 'endereco_nu').val('');
                        $('#' + prefix + 'endereco_co').val('');
                        $('#' + prefix + 'bairro').val('');
                        $('#' + prefix + 'cidade').val('');
                        $('#' + prefix + 'uf').val('');
                        $('#' + prefix + 'pais').val('');
                        ...
                        if (response.logradouro != "") {
                            $('#' + prefix + 'endereco_sn').val(response.logradouro);
                        }
                        if (response.bairro != "") {
                            $('#' + prefix + 'bairro').val(response.bairro);
                        }
                        $('#' + prefix + 'cidade').val(response.localidade);
                        $('#' + prefix + 'uf').val(response.uf);
                        $('#' + prefix + 'pais').val('BRA');
                    }
                }
            });
    ...
    $('.cep').on('input', format_cep);
    $('.cep').on('change', format_cep);
    $('.pais').on('change', function(e) {
        var field = $(this);
        var prefix = field.attr('name').replace(/pais/, '');
        if (field.val() == 'BRA') {
            $('#' + prefix + 'nacional').fadeIn('slow');
            $('#' + prefix + 'cpf_cnpj').focus();
        } else if (field.val() != '') {
            $('#' + prefix + 'nacional').hide();
            $('#' + prefix + 'cpf_cnpj').val('');
            $('#' + prefix + 'ddd_fone').val('');
            $('#' + prefix + 'num_fone').val('');
            $('#' + prefix + 'cep').val('');
    ...
    $('.cc-number').mask('0000 0000 0000 0000 000', {
        translation: {
            '0': {
                pattern: /[0-9]/,
                optional: true
    ...
    $('.cc-exp').mask('00 / 00');
    $('.cc-cvc').mask('#000');
    format_money_str = function(val) {
        val = Number(val.toString().replace(/\D/g, ''));
        if (val == 0) {
            val = 'R$ ';
        } else {
            val = val.toString();
            val = val.replace(/^(\d{1})$/, "0,0$1");
            val = val.replace(/^(\d{2})$/, "0,$1");
            if (val.length > 5) {
                val = val.replace(/(\d)(\d{3})(\d{2})$/, "$1.$2,$3");
            } else {
    ...
    format_money = function(e) {
        $(this).val(format_money_str($(this).val()));
    }
    $('.money').on('input change', format_money);
    $('.phone').mask(function(val) {
        return val.replace(/\D/g, '').length === 9 ? '00000-0000' : '0000-00009';
    });
    $('.fullphone').mask(function(val) {
        return val.replace(/\D/g, '').length === 11 ? '(00) 00000-0000' : '(00) 0000-00009';
    });
    format_cpf_cnpj = function(e) {
        var field = $(this);
        var id = field.attr('id');
        var val = field.val().replace(/\D/g, "");
        if (val.length == 11) {
            $(field).val(val.replace(/(\d{3})(\d{3})(\d{3})(\d{2})/, "$1.$2.$3-$4"));
            if (valid_cpf(val)) {
            ...
            $(field).val(val.replace(/(\d{2})(\d{3})(\d{3})(\d{4})(\d{2})/, "$1.$2.$3/$4-$5"));
            if (valid_cnpj(val)) {
                field.removeClass('invalid');
                $('#' + id + '_invalid').hide();
                field.addClass('valid');
    ...
    $('.cpf_cnpj').on('input change', format_cpf_cnpj);
    $('.cpf').on('input change', format_cpf);
    $('.cnpj').on('input change', format_cnpj);
    format_date = function(e) {
        var field = $(this);
        var id = field.attr('id');
        var val = field.val();
        var birthdate = field.hasClass('birthdate');
        ...
        if (birthdate && valid_birthdate(val)) {
            field.addClass('valid');
            $('#' + id + '_valid').show();
        } else if (!birthdate && valid_date(val)) {
            field.addClass('valid');
            $('#' + id + '_valid').show();
    ...
    $('.date').on('input change', format_date);
    $('.email').mailgun_validator({
        api_key: 'pubkey-7946sfkfycykbgp1d4tz2wsyszqof1f3',
        in_progress: function(e) {
            $(e.target).removeClass('invalid valid');
            $(e.target).addClass('loader');
        },
        error: function(e) {
            $(e.target).removeClass('loader invalid valid');
        },
...
function valid_birthdate(str) {
    if (!valid_date(str))
        return false;
    var bits = str.split('/');
    var now = new Date();
    var year = parseInt(bits[2], 10);
    var month = parseInt(bits[1] - 1, 10);
...
function reset_form(query) {
    $(query).find('input,select,textarea').each(function() {
